...
```bash
PHASE_INFO=$(node ${CLAUDE_PLUGIN_ROOT}/bin/grd-tools.js find-phase "${PHASE}")
```
...
- `phase_name`: Name portion (e.g., "implement-swinir")
...
```bash
PHASE_CHECK=$(node ${CLAUDE_PLUGIN_ROOT}/bin/grd-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ${CLAUDE_PLUGIN_ROOT}/bin/grd-tools.js find-phase "${PHASE}" --raw)
```
