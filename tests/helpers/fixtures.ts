...
function createFixtureDir(): string {
  const tmpRoot: string = fs.mkdtempSync(path.join(os.tmpdir(), 'grd-test-'));
  const dest: string = path.join(tmpRoot, '.planning');
 ...
 *
 * @param dir - Absolute path returned by createFixtureDir
 */
