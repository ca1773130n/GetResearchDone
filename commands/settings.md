...
- `research_gates.interactive.enabled` — human-in-the-loop steering of the research loop (default: `false`)
- `research_gates.interactive.{seed,hypothesize,design,decide}` — per-station pause points; enabled when `interactive.enabled` (defaults: all `true`)
- `research_gates.interactive.fallback` — unattended answerer when no human is present: `"recommended"` (recommended defaults) or `"panel"` (AI discussion panel, degrade-safe → recommended). The loop never pauses unattended either way (default: `"recommended"`)
