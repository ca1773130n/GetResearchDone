...
- **v0.2.2 quickDir Routing Fix & Migration Skill - Phase 37 (active)**
...
### v0.2.2 quickDir Routing Fix & Migration Skill (Phase 37)

**Goal:** Fix quickDir routing bug (hardcoded anonymous instead of current milestone), fix cmdMigrateDirs quick/ routing, and add `/grd:migrate` skill with `grd-migrator` agent for user-facing migration.

---

#### Phase 37: quickDir Routing & Migration Skill

**Goal:** Fix quickDir() to use currentMilestone instead of hardcoded 'anonymous', fix cmdMigrateDirs quick/ routing, and create /grd:migrate skill with grd-migrator agent.

**Dependencies:** None (bugfix milestone)
...
Plans:
- [x] 37-01-PLAN.md — Fix quickDir routing, cmdMigrateDirs quick target, create /grd:migrate skill and grd-migrator agent

**Success Criteria:**
1. `quickDir(cwd)` returns milestone-scoped path for the current milestone (not always anonymous)
2. `quickDir(cwd, milestone)` accepts explicit milestone parameter matching other dir functions
3. `cmdMigrateDirs` routes `quick/` to the current milestone, not hardcoded `anonymous`
4. `/grd:migrate` skill exists with proper frontmatter and process steps
5. `grd-migrator` agent exists with migration rules for complex items
6. All 1,634 tests pass with zero regressions
...
|-------|------|--------|--------------|
| 37 | quickDir Routing & Migration Skill | **Complete** | REQ-70, REQ-71, REQ-72 |
