...
/**
 * Execute a tool command by delegating to grd-tools.js in-process.
 * Falls back to subprocess if in-process delegation fails.
 */
function runToolCommand(
  ...
  const args = buildToolArgs(command, subcommand, extraArgs, jsonFlag);
