...
| `bin/harness_driver.py` | Python 3.11+ I/O driver; binds the pure decision kernel (`autoresearch-core>=0.4.3`) with GRD file I/O; entry point for `gd harness round` subprocess. v0.4.4 adds `UpstreamStore` (emit/read/prune of `UpstreamCandidate` records to `$CLAUDE_PLUGIN_DATA/harness/upstream/`), `CompositeFindings` (local Tesserae findings + pending upstream candidates for the upstream root), and the conservative "about GRD" emit heuristic — all pure I/O; the kernel is unchanged. **v0.4.5:** the round-port classes (`TesseraeFindings`, `AgentProposer`, `RepoEvaluator`, `GitApplier`, `FsRoundStore`, `CompositeFindings`) now explicitly implement autoresearch-core's `@runtime_checkable` port Protocols (`FindingsSource`/`PatchProposer`/`RoundEvaluator`/`Applier`/`RoundStore`); the import guard now requires `autoresearch-core>=0.4.4`. `RepoEvaluator` wraps lint/tsc/jest so a timeout or missing tool no longer crashes the round — failures are caught and classified via autoresearch-core `classify_run_failure` (`[H2]`/`[H3]`/`[H4]`). The harness now consumes Tesserae 0.9.0 AgentRunbook distilled memory: `Runbook`→takeaway / `Gotcha`→insight nodes become evidence (content prefixed `[runbook]`/`[gotcha]`; `Event` nodes skipped); the empty-evidence skip hints at `tesserae config status`. |
