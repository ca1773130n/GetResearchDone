...
| [FLOWS.md](FLOWS.md) | Engineers tracing user commands | Call sequences for 10 key flows (`init`, `plan-phase`, `autopilot`, `evolve`, etc.) | ~2,600 words |
...
- **~85 config fields** across `GrdConfig`, `SchedulerConfig`, `SuperpowersConfig`, `CleanupConfig`, `DiscussionConfig`, `CeremonyConfig`, `EvolveConfig`, and agent frontmatter.
