...
| [FLOWS.md](FLOWS.md) | Engineers tracing user commands | Call sequences for 10 key flows (`init`, `plan-phase`, `autopilot`, `harness round`, etc.); `gd evolve` flow retained as deprecated historical reference | ~2,600 words |
...
- **~85 config fields** across `GrdConfig`, `SchedulerConfig`, `SuperpowersConfig`, `CleanupConfig`, `DiscussionConfig`, `CeremonyConfig`, `HarnessConfig` (v0.4.3+), `EvolveConfig` (deprecated v0.4.3), and agent frontmatter.
