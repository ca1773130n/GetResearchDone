...
/**
 * Milestone-scoped codebase analysis directory with backward-compatible fallback.
 *
 * Returns the new-style path when the milestone directory exists on disk,
 * otherwise falls back to .planning/codebase/.
 *
 * @param {string} cwd - Project working directory
 * @param {string} [milestone] - Milestone version; defaults to currentMilestone(cwd)
 * @returns {string} Absolute path to codebase directory (new or old layout)
 */
...
/**
 * Quick tasks directory with backward-compatible fallback.
 *
 * Quick tasks are milestone-independent. Returns the new-style path
 * (.planning/milestones/anonymous/quick/) when the anonymous milestone
 * directory exists on disk, otherwise falls back to .planning/quick/.
 *
 * @param {string} cwd - Project working directory
 * @returns {string} Absolute path to quick tasks directory (new or old layout)
 */
function quickDir(cwd) {
  if (milestoneExistsOnDisk(cwd, 'anonymous')) {
