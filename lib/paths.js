 ...
 *
 * Strategy 1: Parse ROADMAP.md for an active milestone version.
 * Strategy 2: Scan milestones/ for a single active (non-archived) directory.
 * Returns 'anonymous' when neither strategy succeeds.
 *
 * @param {string} cwd - Project working directory
 * @returns {string} Milestone version or 'anonymous'
 */
function inferMilestoneFromDisk(cwd) {
  // Strategy 1: Parse ROADMAP.md for active milestone
  try {
    const roadmap = fs.readFileSync(path.join(cwd, '.planning', 'ROADMAP.md'), 'utf-8');
    // Look for "(in progress)" milestone bullet
    const inProgress = roadmap.match(/-\s+(v[\d.]+)\s+[^\n(]+?\s*\(in progress\)/im);
    if (inProgress) return inProgress[1];
    // Last non-shipped milestone bullet
  ...
  // Strategy 2: Single active milestone directory on disk
