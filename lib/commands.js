...
// Module-level cache for roadmap content reads across command calls.
// Prevents redundant disk reads when multiple commands access ROADMAP.md in the same process.
const _roadmapContentCache = new Map();
function readCachedRoadmap(roadmapPath) {
  ...
  return _roadmapContentCache.get(roadmapPath);
}

// Module-level cache for STATE.md reads across command calls.
// Prevents redundant disk reads when multiple commands access STATE.md in the same process.
const _stateContentCache = new Map();
function readCachedState(statePath) {
  if (!_stateContentCache.has(statePath)) {
    const content = safeReadFile(statePath);
    if (content !== null) _stateContentCache.set(statePath, content);
    return content;
  }
  return _stateContentCache.get(statePath);
}
...
function cmdGenerateSlug(text, raw) {
  if (!text) {
    error('text required for slug generation. Usage: generate-slug <text>. Provide a text string, e.g.: generate-slug "my phase name"');
  }
    ...
    for (const file of files) {
      try {
        ...
        if (readErr && readErr.code && readErr.code !== 'ENOENT') {
          process.stderr.write(`[todos] error reading ${file} (${readErr.code}): ${readErr.message}\n`);
        }
      }
    }
  } catch {
 ...
 */
function cmdTodoComplete(cwd, filename, raw, dryRun) {
  if (!filename) {
    error('filename required for todo complete. Usage: todos complete <filename> (run "todos list" to see pending filenames)');
  }
 ...
 * @param {boolean} [dryRun=false] - If true, report what would be checked without side effects
 * @returns {void} Outputs existence check result to stdout and exits
 */
function cmdVerifyPathExists(cwd, targetPath, raw, dryRun) {
  if (!targetPath) {
  ...
  if (dryRun) {
    output({ dry_run: true, path: targetPath, full_path: fullPath }, raw, `dry-run: would check ${targetPath}`);
    return;
  }
 ...
 */
function cmdConfigEnsureSection(cwd, raw, dryRun) {
 ...
 * @returns {void} Outputs result to stdout and exits
 */
function cmdConfigSet(cwd, keyPath, value, raw, dryRun) {
...
function cmdHistoryDigest(cwd, raw) {
  ...
  try {
    ...
    const techList = digest.tech_stack.length > 0 ? digest.tech_stack.join(', ') : 'none';
...
function cmdPhasePlanIndex(cwd, phase, raw) {
  ...
  output(result, raw, `Phase ${result.phase}: ${result.plans.length} plans, ${result.incomplete.length} incomplete`);
}
...
function cmdSummaryExtract(cwd, summaryPath, fields, raw) {
  ...
  if (fields && fields.length > 0) {
    ...
    output(filtered, raw, filtered.one_liner || path.basename(summaryPath));
    return;
  }
...
function cmdProgressRender(cwd, format, raw) {
  ...
  try {
    ...
    // Phases directory may not exist yet; proceed with empty progress
  }
    ...
    if (blockersSection) {
      ...
      if (blockersText && blockersText !== 'None.' && blockersText !== 'None') {
        ...
        for (const item of items) {
          blockerItems.push(item.replace(/^-\s+/, '').trim());
        }
      }
    }
  } catch {
    // STATE.md may not exist or be unparseable; skip blockers
  }
  ...
  if (format === 'table') {
    ...
    // Show BLOCKED warning when there are active blockers
    ...
    // JSON format — raw mode returns human-readable summary
...
function parseDashboardStateSummary(stateContent) {
  ...
  if (blockersSection) {
    ...
    blockerItems = filtered.map((item) => item.replace(/^-\s+/, '').trim());
  }
...
function renderDashboardTui(
  ...
  if (stateSummary.activeBlockers > 0 && stateSummary.blockerItems && stateSummary.blockerItems.length > 0) {
    tui += '## ACTIVE BLOCKERS\n\n';
    for (const blocker of stateSummary.blockerItems) {
      tui += `  ! ${blocker}\n`;
    }
    tui += '\n';
  }
...
/**
 * Build dashboard data from ROADMAP.md and STATE.md.
 * Returns null if ROADMAP.md is missing, otherwise returns a plain data object
 * with milestones, phases, state summary, and aggregate counts.
 * @param {string} cwd - Project working directory
 * @returns {Object|null} Dashboard data or null if ROADMAP.md not found
 */
function buildDashboardData(cwd) {
 ...
 * @param {Object} data - Dashboard data returned by buildDashboardData()
 * @param {Object} [options] - Render options (e.g., { filter: 'incomplete' })
 * @returns {{ jsonResult: Object, tui: string }} Rendered output
 */
function renderDashboard(data, options) {
  ...
  const jsonResult = {
    milestones: milestones.map((ms) => {
      let phases = ms.phases.map(({ plan_files, summary_files, ...rest }) => rest);
 ...
 * @param {Object} [options] - Options (e.g., { filter: 'incomplete' })
 * @returns {void} Outputs dashboard to stdout and exits
 */
function cmdDashboard(cwd, raw, options) {
  ...
  if (!raw && tui) {
    output(jsonResult, true, tui);
    return;
  }
  output(jsonResult, raw, filter !== null ? undefined : tui);
}
...
function cmdPhaseDetail(cwd, phase, raw) {
  ...
  try {
    ...
    // Phases directory may not exist; phase will be reported as not found
  }
  ...
  const phaseHeadingPattern = new RegExp(
    `#{2,} Phase ${phaseNumber.replace('.', '\\.')}:([\\s\\S]*?)(?=#{2,} Phase |$)`,
    'i'
  );
  ...
  if (phaseSection) {
    ...
    if (reqLineMatch) {
      ...
      if (reqIds.length > 0) {
        const requirementsPath = path.join(getPlanningDir(cwd), 'REQUIREMENTS.md');
...
function cmdLongTermRoadmap(cwd, subcommand, args, raw) {
  ...
  switch (subcommand) {
    ...
    case 'add': {
      ...
      output(
        { content: result.content, path: path.relative(cwd, path.join(getPlanningDir(cwd), 'LONG-TERM-ROADMAP.md')), id: result.id },
        raw,
        result.content
      );
      break;
    ...
    case 'remove': {
      ...
      if (result && typeof result === 'object' && result.error) {
        ...
        output(
          { content: result, path: path.relative(cwd, path.join(getPlanningDir(cwd), 'LONG-TERM-ROADMAP.md')), removed: id },
          raw,
          result
        );
      }
    ...
    case 'update': {
      ...
      if (result && typeof result === 'object' && result.error) {
        ...
        output(
          {
            content: result,
            path: path.relative(cwd, path.join(getPlanningDir(cwd), 'LONG-TERM-ROADMAP.md')),
            id,
    ...
    case 'link': {
      ...
      if (result && typeof result === 'object' && result.error) {
        ...
        output(
          { content: result, path: path.relative(cwd, path.join(getPlanningDir(cwd), 'LONG-TERM-ROADMAP.md')), id, linked: version },
          raw,
          result
        );
      }
    ...
    case 'unlink': {
      ...
      if (result && typeof result === 'object' && result.error) {
        ...
        output(
          { content: result, path: path.relative(cwd, path.join(getPlanningDir(cwd), 'LONG-TERM-ROADMAP.md')), id, unlinked: version },
          raw,
          result
        );
      }
    ...
    case 'init': {
      ...
      output({ content, path: path.relative(cwd, path.join(getPlanningDir(cwd), 'LONG-TERM-ROADMAP.md')) }, raw, content);
      break;
    }
    case 'history': {
...
// ─── Requirements ────────────────────────────────────────────────────────────
// All requirement management functions are defined in lib/requirements.js and
// imported at the top of this file. They are re-exported via module.exports below.
 ...
 * @returns {void} Outputs search matches to stdout and exits
 */
 ...
 */
function cmdMigrateDirs(cwd, raw, dryRun) {
  ...
  for (const entry of migrationMap) {
    ...
    if (dryRun) {
      movedDirectories.push({
        ...
        entries_to_move: contents.filter((item) => !fs.existsSync(path.join(targetDir, item))).length,
      });
      continue;
    }
 ...
 * @returns {void} Outputs coverage report to stdout and exits
 */
function cmdCoverageReport(cwd, options, raw) {
  ...
  try {
    ...
    // Cache the file content to avoid multiple reads of the same file
    summaryContent = fs.readFileSync(summaryPath, 'utf-8');
    coverageData = JSON.parse(summaryContent);
  } catch (_e) {
    // Jest may exit non-zero if tests fail but still produce coverage; reuse cached content if available
    if (fs.existsSync(summaryPath)) {
      summaryContent = summaryContent || fs.readFileSync(summaryPath, 'utf-8');
      coverageData = JSON.parse(summaryContent);
    } else {
 ...
 * @param {boolean} raw - If true, output raw text instead of JSON
 * @returns {void} Outputs health check results to stdout and exits
 */
