...
function cmdConfigEnsureSection(cwd, raw) {
  ...
  try {
    ...
    const result = { created: true, path: path.relative(cwd, path.join(getPlanningDir(cwd), 'config.json')) };
...
function cmdPhasePlanIndex(cwd, phase, raw) {
  if (!phase) {
    error('phase required for phase-plan-index. Usage: phase-plan-index <phase-number>. Provide a phase number, e.g.: phase-plan-index 2');
  }
