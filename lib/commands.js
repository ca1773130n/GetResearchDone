...
function cmdDashboard(cwd, raw) {
  ...
  // Parse shipped milestones from "## Milestones" bullet list
  // Format: - vX.Y.Z Name - Phases N-M (shipped YYYY-MM-DD)
  const shippedMilestones = [];
  const milestonesSection = roadmapContent.match(/^##\s+Milestones\s*\n([\s\S]*?)(?=\n##\s|\n$)/m);
  if (milestonesSection) {
    // Match with phase range: - vX.Y.Z Name - Phases N-M (shipped YYYY-MM-DD)
    const shippedWithPhasesRegex =
      /^-\s+(v[\d.]+)\s+(.+?)\s*-\s*Phases\s+(\d+)-(\d+)\s*\(shipped\s+(\d{4}-\d{2}-\d{2})\)/gm;
    // Match without phase range: - vX.Y.Z Name (shipped YYYY-MM-DD)
    const shippedNoPhasesRegex =
      /^-\s+(v[\d.]+)\s+(.+?)\s*\(shipped\s+(\d{4}-\d{2}-\d{2})\)/gm;
    const seen = new Set();
    let sMatch;
    while ((sMatch = shippedWithPhasesRegex.exec(milestonesSection[1])) !== null) {
      ...
      shippedMilestones.push({
        ...
        target: null,
        status: 'shipped',
        shipped_date: sMatch[5],
        phase_range: `${startPhase}-${endPhase}`,
        phase_count: endPhase - startPhase + 1,
        progress_percent: 100,
        phases: [],
      });
    }
    while ((sMatch = shippedNoPhasesRegex.exec(milestonesSection[1])) !== null) {
      if (seen.has(sMatch[1])) continue; // already matched with phases
      shippedMilestones.push({
        name: sMatch[2].trim(),
        number: null,
        version: sMatch[1],
  ...
  // Parse active milestones from ROADMAP.md (## headings)
  ...
  while ((mMatch = milestoneRegex.exec(roadmapContent)) !== null) {
    ...
    // Skip the "## Milestones" and "## Phases" section headers
  ...
  // Combined milestones: shipped first, then active
  ...
  while ((pMatch = phaseRegex.exec(roadmapContent)) !== null) {
    ...
    // Determine which active milestone this phase belongs to
    let activeMsIdx = 0;
    ...
    if (activeMilestones[activeMsIdx]) {
      activeMilestones[activeMsIdx].phases.push(phaseData);
    }
  }

  // Calculate progress for each active milestone (per-phase weighting: 0-plan phases count as 0%)
  ...
  // Build timeline from active milestones with dates
  ...
  // Render shipped milestones as compact summary
  ...
  // Render active milestones with full detail
  for (const ms of activeMilestones) {
  ...
  // Timeline section (active milestones only)
  ...
  switch (subcommand) {
    ...
    case 'update': {
      ...
      if (result && typeof result === 'object' && result.error) {
        ...
        output(
          { content: result, path: '.planning/LONG-TERM-ROADMAP.md', id, updated_fields: Object.keys(updates) },
          raw,
          result
        );
      }
    ...
    case 'init': {
      ...
      output(
        { content, path: '.planning/LONG-TERM-ROADMAP.md' },
        raw,
        content
      );
      break;
    ...
    case 'history': {
      ...
      output(
        { content: histResult, path: '.planning/LONG-TERM-ROADMAP.md', action: histAction, details: histDetails },
        raw,
        histResult
      );
      break;
    ...
    case 'parse': {
      const filePath = args[0]
        ? path.isAbsolute(args[0]) ? args[0] : path.join(cwd, args[0])
        : ltrmPath;
