...
function cmdDetectBackend(cwd, raw) {
  ...
  const models = {
    opus: resolveBackendModel(backend, 'opus', config),
    sonnet: resolveBackendModel(backend, 'sonnet', config),
    haiku: resolveBackendModel(backend, 'haiku', config),
  };

  const capabilities = getBackendCapabilities(backend);

  const result = { backend, models, capabilities };
  ...
  switch (subcommand) {
    case 'parse': {
      ...
      if (raw) {
        ...
        output(parsed, raw, `${total} milestones (${nowCount} now, ${nextCount} next, ${laterCount} later)`);
      } else {
        output(parsed, raw);
      }
      break;
    ...
    case 'validate': {
      ...
      if (raw) {
        const rawText = validation.valid
          ? 'valid'
    ...
    case 'display': {
      ...
      if (raw) {
        output({ formatted, milestone_count: milestoneCount, mode: 'hierarchical' }, raw, formatted);
      } else {
    ...
    case 'refine': {
      ...
      if (refResult && typeof refResult === 'object' && refResult.error) {
        ...
        output({ content: refResult, path: '.planning/LONG-TERM-ROADMAP.md', version: refVersion, updated_fields: Object.keys(updates) }, raw, refResult);
      }
      break;
    }
    case 'promote': {
      ...
      if (proResult && typeof proResult === 'object' && proResult.error) {
        ...
        output({ content: proResult, path: '.planning/LONG-TERM-ROADMAP.md', version: proVersion, new_tier: newTier }, raw, proResult);
      }
      break;
    }
    case 'history': {
      ...
      output({ content: histResult, path: '.planning/LONG-TERM-ROADMAP.md', action: histAction, details: histDetails }, raw, histResult);
      break;
    }
    default:
