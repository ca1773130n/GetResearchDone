...
function cmdGenerateSlug(text, raw) {
  if (!text) {
    error('text required for slug generation. Usage: generate-slug <text>');
  }
...
function cmdPhasePlanIndex(cwd, phase, raw) {
  if (!phase) {
    error('phase required for phase-plan-index. Usage: phase-plan-index <phase-number>');
  }
...
function cmdSummaryExtract(cwd, summaryPath, fields, raw) {
  if (!summaryPath) {
    error('summary-path required for summary-extract. Usage: summary-extract <path-to-SUMMARY.md>');
  }
...
function cmdLongTermRoadmap(cwd, subcommand, args, raw) {
  ...
  /** Read ROADMAP.md, caching result; returns content or null */
  let _roadmapContent;
  function readRoadmap() {
    if (_roadmapContent === undefined) _roadmapContent = safeReadFile(roadmapPath);
...
function cmdQualityAnalysis(cwd, args, raw) {
  ...
  if (!phaseNum) {
    error('--phase flag required for quality-analysis. Usage: quality-analysis --phase <phase-number>');
  }
