...
function cmdInitExecutePhase(cwd, phase, includes, raw) {
  ...
  const result = {
    ...
    // Branch name (pre-computed — always the phase branch, regardless of strategy)
    branch_name:
    ...
    // Strategy-aware PR target branch (where PRs should point)
    ...
    // Milestone branch name (only set when strategy is 'milestone')
    ...
    worktree_path: phaseInfo
      ? path.resolve(
          ...
          `${milestone.version}-${phaseInfo.phase_number}`
        )
      : null,
    worktree_branch:
      config.branching_strategy !== 'none' && phaseInfo
