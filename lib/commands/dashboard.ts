...
const {
  ...
  milestonesDir: getMilestonesDir,
}: {
...
function renderDashboardTui(
  ...
  if (shippedMilestones.length > 0) {
    ...
    for (const ms of shippedMilestones) {
      const phaseInfo =
        ms.phase_count && ms.phase_count > 0 ? ` \u2014 ${ms.phase_count} phases` : '';
  ...
  if (timelineMilestones.length > 0) {
    ...
    const dateToPos = (dateStr: string): number => {
      ...
      return Math.round(
        ((d.getTime() - minDate.getTime()) / (1000 * 60 * 60 * 24) / totalDays) * tlBarWidth
      );
    };
  ...
  if (
    stateSummary.activeBlockers > 0 &&
    stateSummary.blockerItems &&
    stateSummary.blockerItems.length > 0
  ) {
    tui += '## ACTIVE BLOCKERS\n\n';
...
function renderDashboard(
  ...
  const jsonResult: Record<string, unknown> = {
    milestones: milestones.map((ms) => {
      let phases: Record<string, unknown>[] = ms.phases.map(
        ({ plan_files: _pf, summary_files: _sf, ...rest }) => rest
      );
...
function cmdPhaseDetail(cwd: string, phase: string, raw: boolean): void {
  ...
  try {
    ...
    /* Phases directory may not exist */
  }
  ...
  for (const planFile of planFiles) {
    ...
    if (hasSummary) {
      const summaryContent: string =
        safeReadFile(path.join(phaseDir, planId + '-SUMMARY.md')) || '';
      ...
      if (summaryFm['key-files']) {
        ...
        const created = kf.created
          ? Array.isArray(kf.created)
            ? (kf.created as string[])
            : [kf.created as string]
          : [];
        const modified = kf.modified
          ? Array.isArray(kf.modified)
            ? (kf.modified as string[])
            : [kf.modified as string]
          : [];
      ...
      if (summaryFm['key-decisions'] && Array.isArray(summaryFm['key-decisions']))
        allDecisions.push(...(summaryFm['key-decisions'] as string[]));
      if (duration) {
    ...
    plans.push({
      id: planId,
      wave: parseInt(String(planFm.wave), 10) || 1,
      type: (planFm.type as string) || 'execute',
  ...
  if (decisionsSection) {
    ...
    for (const row of rows) {
      const cells = row
        ...
        .map((c: string) => c.trim())
        .filter(Boolean);
      if (cells.length >= 2) stateDecisions.push(cells[1]);
    }
  }
        ...
        if (reqContent) {
          ...
          for (const reqId of reqIds) {
            const req = allReqs.find(
              (r: { id: string }) => r.id.toLowerCase() === reqId.toLowerCase()
            );
            const matrixRow = matrix.find(
              (m: { req: string }) => m.req.toLowerCase() === reqId.toLowerCase()
            );
