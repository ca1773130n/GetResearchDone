 ...
 * @param {{ threshold?: number, onProgress?: function }} [options={}] - Options with optional complexity threshold (default 10) and progress callback
...
function analyzeComplexity(cwd, files, options = {}) {
  ...
  try {
    ...
    if (err.stdout) {
      try {
        const violations = parseEslintComplexityResults(cwd, err.stdout);
        if (!onProgress) _complexityCache.set(cacheKey, violations);
        if (onProgress) {
          onProgress({ event: 'completed', violation_count: violations.length });
        }
    ...
    process.stderr.write('[cleanup] ESLint complexity analysis failed: ' + (err.message || 'unknown error') + '\n');
    return [];
  }
}
 ...
 * @param {string} block - The full JSDoc comment text
 * @returns {string[]} Array of parameter names documented with @param
 */
 ...
 * @param {string} contentAfterBlock - File content starting right after the JSDoc block
 * @returns {{ funcName: string|null, paramsStr: string|null }}
 */
    ...
    while ((blockMatch = blockRe.exec(content)) !== null) {
      ...
      for (const dp of docParams) {
        if (!actualParams.includes(dp)) {
          issues.push({ file, line: blockStartLine, functionName: funcName, issue: `extra @param: ${dp}` });
        }
      }
      for (const ap of actualParams) {
        if (!docParams.includes(ap)) {
          issues.push({ file, line: blockStartLine, functionName: funcName, issue: `missing @param: ${ap}` });
        }
      }
    }
  }
...
function analyzeTestCoverageGaps(cwd, files) {
  ...
  for (const file of files) {
    ...
    if (!testContent) {
      for (const exportName of exportNames) {
        gaps.push({ file, exportName, testFile, line: findExportLine(content, exportName) });
      }
      continue;
    }
 ...
 * @param {string} cwd - Project working directory
 * @returns {Object} Map of phase number to quality summary
 */
 ...
 * @param {Object} summary - Quality summary object
 */
 ...
 * @param {Object} current - Current quality summary
 * @param {Object} previous - Previous quality summary
 * @param {string} fromPhase - Phase number the previous data came from
 * @returns {Object} Map of field name to { delta, label }
 */
function computeTrends(current, previous, fromPhase) {
  const trends = {};
  for (const key of Object.keys(current)) {
    if (typeof current[key] !== 'number' || typeof previous[key] !== 'number') continue;
