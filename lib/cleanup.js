...
const { safeReadFile, safeReadJSON } = require('./utils');

// ─── File Content Cache ───────────────────────────────────────────────────────
let _cleanupRunCache = null;

function _cachedRead(absPath) {
  if (!_cleanupRunCache) return safeReadFile(absPath);
  if (!_cleanupRunCache.has(absPath)) _cleanupRunCache.set(absPath, safeReadFile(absPath));
  return _cleanupRunCache.get(absPath);
}
