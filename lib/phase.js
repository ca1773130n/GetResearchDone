...
function cmdPhaseAdd(cwd, description, raw) {
  ...
  // Find highest integer phase number (in active section only)
...
function cmdMilestoneComplete(cwd, version, options, raw) {
  ...
  const phasesArchiveDir = path.join(archiveDir, `${version}-phases`);
...
function cmdValidateConsistency(cwd, raw) {
  ...
  output(
    { passed, errors: errors_list, warnings, warning_count: warnings.length },
    raw,
...
function cmdVersionBump(cwd, version, raw) {
  if (!version) {
    error('version required for version bump (e.g., v1.0.0)');
  }
