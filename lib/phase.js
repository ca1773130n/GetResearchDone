...
function cmdPhaseAdd(cwd, description, raw, context) {
  ...
  if (description.length > 60) {
    error(`description too long (${description.length} chars): must not exceed 60 characters. Shorten the description to 60 characters or fewer before retrying.`);
  }
...
function cmdPhaseRemove(cwd, targetPhase, options, raw) {
  if (!targetPhase) {
    error('phase number required for phase remove. Usage: phase remove <phase-number>. Provide the phase number to remove, e.g.: phase remove 3');
  }
...
function cmdPhaseComplete(cwd, phaseNum, raw, options) {
  if (!phaseNum) {
    error('phase number required for phase complete. Usage: phase complete <phase-number>. Provide the phase number to complete, e.g.: phase complete 2');
  }
...
function cmdMilestoneComplete(cwd, version, options, raw) {
  if (!version) {
    error('version required for milestone complete (e.g., v1.0). Usage: milestone complete <version>. Provide the milestone version to complete, e.g.: milestone complete v1.0');
  }
...
function cmdVersionBump(cwd, version, raw) {
  if (!version) {
    error('version required for version bump (e.g., v1.0.0). Usage: milestone version-bump <version>. Provide a semver version, e.g.: milestone version-bump v1.2.0');
  }
