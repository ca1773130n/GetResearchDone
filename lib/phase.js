...
const _roadmapFileCache = new Map();
function readRoadmapFile(roadmapPath) {
  if (!_roadmapFileCache.has(roadmapPath)) {
    _roadmapFileCache.set(roadmapPath, fs.readFileSync(roadmapPath, 'utf-8'));
  }
  return _roadmapFileCache.get(roadmapPath);
}
function writeRoadmapFile(roadmapPath, content) {
...
function readStateFile(statePath) {
  ...
  return _stateFileCache.get(statePath);
}
function writeStateFile(statePath, content) {
 ...
 * @param {string} targetPhase - Phase number to validate
 ...
 * @param {Array<{from:string,to:string}>} renamedDirs - Accumulator for renamed directories
 * @param {Array<{from:string,to:string}>} renamedFiles - Accumulator for renamed files
 * @returns {void}
 */
 ...
 * Mutates renamedDirs and renamedFiles arrays in place.
 * @param {string} phasesDir - Absolute path to the phases directory
 * @param {string} baseInt - The integer part of the removed decimal phase (e.g. "06")
 * @param {number} removedDecimal - The decimal part that was removed (e.g. 2 for "06.2")
 * @param {Array<{from:string,to:string}>} renamedDirs - Accumulator for renamed directories
 * @param {Array<{from:string,to:string}>} renamedFiles - Accumulator for renamed files
 * @returns {void}
 */
 ...
 * @param {string} phasesDir - Absolute path to the phases directory
 * @param {string} normalized - Normalized phase number string (e.g. "06" or "06.2")
 ...
 * @param {string} roadmapContent - Current ROADMAP.md content
 * @param {string} targetPhase - The phase number that was removed (e.g. "06" or "06.2")
 * @param {string} normalized - Normalized phase number string
 * @param {boolean} isDecimal - Whether the removed phase was a decimal phase
 * @returns {string} Updated roadmap content
 */
 ...
 * @param {string} _phasesDir - Absolute path to the phases directory
 * @param {string} _oldNum - Old phase number string
 * @param {string} _newNum - New phase number string
 * @returns {void}
 */
 ...
 * @returns {void} Outputs result to stdout and exits
 */
function _phaseCompleteCore(cwd, phaseNum, options) {
  ...
  if (!gates.passed) {
    return { gate_failed: true, gate_errors: gates.errors, gate_warnings: gates.warnings };
  }
  ...
  if (fs.existsSync(roadmapPath)) {
    let roadmapContent = readRoadmapFile(roadmapPath);
 ...
 * @param {string} cwd - Project working directory
 * @param {string} phaseNum - Phase number to complete (e.g., '02' or '2')
 * @param {boolean} raw - Output raw text instead of JSON
 * @param {Object} [options] - Options (e.g., { dryRun: true, force: false })
 * @returns {void} Outputs completion result to stdout and exits
 */
function cmdPhaseComplete(cwd, phaseNum, raw, options) {
  ...
  if (raw) {
    if (result.dry_run) {
      ...
      if (result.quality_report && result.quality_report.summary.total_issues > 0) {
 ...
 * @param {string} sourceDir - Directory containing the phase subdirectories to archive
 * @param {string} archiveDir - Destination directory for archived files
 * @param {Object} ctx - Additional context: { roadmapPath, reqPath, milestoneName, today, phasesDir, phaseCount, totalPlans, totalTasks, accomplishments, phasesAlreadyInPlace }
 * @returns {{ archivedPhaseCount: number }}
 */
function _archiveMilestone(cwd, version, sourceDir, archiveDir, ctx) {
  const {
    roadmapPath,
    reqPath,
    milestoneName,
 ...
 * @returns {boolean} Whether STATE.md was updated
 */
 ...
 * @param {string} milestonesPath - Absolute path to MILESTONES.md
 * @param {string} version - Milestone version string (e.g. 'v1.0')
 * @param {string} milestoneName - Display name for the milestone
 * @param {string} today - ISO date string for today (YYYY-MM-DD)
 * @param {number} phaseCount - Number of phases completed
 * @param {number} totalPlans - Total plans executed
 * @param {number} totalTasks - Total tasks executed
 * @param {string[]} accomplishments - List of one-liner accomplishment strings
 * @returns {void}
 */
...
function cmdVersionBump(cwd, version, raw) {
  if (!version) {
    error('version required for version bump (e.g., v1.0.0). Usage: milestone version-bump <version>. Provide the new version, e.g.: milestone version-bump v1.3.0. Current version can be found in .planning/config.json.');
  }
