  ...
  try {
    ...
    if (config.tracker) {
      ...
      if (tracker.provider === 'github' && tracker.github) {
        if (!tracker.github.project_board) {
    ...
    process.stderr.write('Warning: failed to parse config.json: ' + (e.message || 'unknown error') + '\n');
    return { provider: 'none' };
  }
}
 ...
 * @param {string} row - A single markdown table row string
 ...
 * @param {Object} mapping - Parsed mapping object with milestones, phases, and plans
 * @returns {Map<string, Object>} Index map keyed by identifier
 */
 ...
 * @param {string} cwd - Project working directory
 * @returns {Object} Mapping object with provider, last_synced, milestones, phases, plans, and _trackerIndex
 */
function loadTrackerMapping(cwd) {
  ...
  if (!content) {
    const empty = { provider: null, last_synced: null, milestones: {}, phases: {}, plans: {} };
    empty._trackerIndex = new Map();
...
function handleSyncRoadmap(cwd, args, raw) {
  ...
  if (!tracker) {
    output({ error: 'No tracker configured', created: 0, updated: 0, skipped: 0, errors: 0 }, raw);
    return;
  }
  const roadmapContent = safeReadMarkdown(path.join(cwd, '.planning', 'ROADMAP.md'));
  if (!roadmapContent) {
    output({ error: 'No ROADMAP.md found', created: 0, updated: 0, skipped: 0, errors: 0 }, raw);
    return;
  }
  ...
  while ((match = phaseRegex.exec(activeContent)) !== null) {
    ...
    const afterPhase = activeContent.slice(
  ...
  if (isDryRun) {
    ...
    for (const p of phases) {
      if (mapping.phases && mapping.phases[p.number]) {
        wouldSkip.push({ number: p.number, name: p.name, reason: 'already_mapped' });
      } else {
        wouldCreate.push({ number: p.number, name: p.name });
      }
    }
    output({ dry_run: true, would_create: wouldCreate, would_skip: wouldSkip, created: 0 }, raw);
    return;
  }
...
function handleUpdateStatus(cwd, args, raw) {
  ...
  const status = args[1];
  if (!phaseNum || !status) {
    error('Usage: tracker update-status <phase-number> <status>. Provide phase number and one of the valid status values (in-progress, completed, blocked). Example: tracker update-status 3 completed');
  }
  const config = loadTrackerConfig(cwd);
  if (config.provider === 'mcp-atlassian') {
    const mapping = loadTrackerMapping(cwd);
 ...
 * @param {string} content - Roadmap content with shipped sections already stripped
 * @returns {Array<{heading: string, version: string, index: number}>} Ordered milestone list
 */
 ...
 * @param {string} content - Roadmap content with shipped sections already stripped
 * @param {Array<{version: string, index: number}>} milestonePositions - Parsed milestone list
 * @returns {Array<{number: string, name: string, goal: string, milestone: string|null, index: number}>}
 */
function _parseAllPhases(content, milestonePositions) {
  ...
  while ((match = phaseRegex.exec(content)) !== null) {
    const number = match[1];
    const name = match[2].trim();
    const afterPhase = content.slice(
      match.index + match[0].length,
      match.index + match[0].length + 500
    );
    const goalMatch = afterPhase.match(/(?:\*\*Goal:\*\*|Goal:)\s*(.+)/);
 ...
 * @param {Array<{heading: string, version: string, index: number}>} milestones - Parsed milestones
 * @param {Array<{number: string, name: string, goal: string, milestone: string|null}>} phases - Parsed phases
 * @param {{mapping: Object, schedule: Object}} ctx - Loaded mapping and computed schedule
 * @returns {Array<Object>} Operations array (create/skip entries)
 */
...
function handlePrepareRoadmapSync(cwd, _args, raw) {
  ...
  const operations = _buildMilestoneOperations(milestonePositions, allPhases, { mapping, schedule });
...
function handlePreparePhaseSync(cwd, args, raw) {
  const phaseNum = args[0];
  if (!phaseNum) {
    error('Usage: tracker prepare-phase-sync <phase-number>. Provide the phase number to sync, e.g.: tracker prepare-phase-sync 3');
  }
  const config = loadTrackerConfig(cwd);
  if (config.provider !== 'mcp-atlassian') {
    output(
