...
function handleSyncPhase(cwd, args, raw) {
  ...
  if (!phaseNum) {
    error('Usage: tracker sync-phase <phase-number>. Example: tracker sync-phase 2');
  }
  const config = loadTrackerConfig(cwd);
  if (config.provider === 'mcp-atlassian') {
    output(
...
function handleUpdateStatus(cwd, args, raw) {
  ...
  const status = args[1];
  if (!phaseNum || !status) {
    error('Usage: tracker update-status <phase-number> <status>. Example: tracker update-status 2 in_progress');
  }
  const config = loadTrackerConfig(cwd);
  if (config.provider === 'mcp-atlassian') {
    const mapping = loadTrackerMapping(cwd);
...
function handlePreparePhaseSync(cwd, args, raw) {
  const phaseNum = args[0];
  if (!phaseNum) {
    error('Usage: tracker prepare-phase-sync <phase-number>. Example: tracker prepare-phase-sync 2');
  }
  const config = loadTrackerConfig(cwd);
  if (config.provider !== 'mcp-atlassian') {
    output(
...
function handleRecordMapping(cwd, args, raw) {
  ...
  if (!type || !issueKey) {
    error(
      'Usage: tracker record-mapping --type milestone|phase|plan [--milestone V] [--phase N] [--plan M] --key PROJ-1 --url URL [--parent PROJ-0]. Example: tracker record-mapping --type phase --phase 2 --key PROJ-5 --url https://...'
    );
  }
...
function handleRecordStatus(cwd, args, raw) {
  ...
  if (!phaseNum || !status) {
    error('Usage: tracker record-status --phase N --status pending|in_progress|complete. Example: tracker record-status --phase 2 --status in_progress');
  }
