...
function handleSyncPhase(cwd, args, raw) {
  ...
  if (!phaseNum) {
    error('Usage: tracker sync-phase <phase-number>. Example: tracker sync-phase 2. Provide the phase number to sync, e.g.: tracker sync-phase 2');
  }
  const config = loadTrackerConfig(cwd);
  if (config.provider === 'mcp-atlassian') {
    output(
...
function handleUpdateStatus(cwd, args, raw) {
  ...
  const status = args[1];
  if (!phaseNum || !status) {
    error('Usage: tracker update-status <phase-number> <status>. Example: tracker update-status 2 in_progress. Provide both phase number and status, e.g.: tracker update-status 2 in_progress');
  }
  const config = loadTrackerConfig(cwd);
  if (config.provider === 'mcp-atlassian') {
    const mapping = loadTrackerMapping(cwd);
...
function handlePreparePhaseSync(cwd, args, raw) {
  const phaseNum = args[0];
  if (!phaseNum) {
    error('Usage: tracker prepare-phase-sync <phase-number>. Example: tracker prepare-phase-sync 2. Provide the phase number, e.g.: tracker prepare-phase-sync 2');
  }
  const config = loadTrackerConfig(cwd);
  if (config.provider !== 'mcp-atlassian') {
    output(
