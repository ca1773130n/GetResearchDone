...
const { loadConfig, normalizePhaseName, safeReadFile, stripShippedSections, createRunCache } = require('./utils');
const { phasesDir: getPhasesDirPath } = require('./paths');

// ─── File Content Cache ───────────────────────────────────────────────────────
const _gatesCache = createRunCache();

function _gatesCachedRead(p) {
  return _gatesCache.get(p, safeReadFile);
}
