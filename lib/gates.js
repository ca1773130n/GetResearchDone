...
const { loadConfig, normalizePhaseName, safeReadFile } = require('./utils');
...
function checkPhaseInRoadmap(cwd, phase) {
  ...
  if (!phaseRegex.test(roadmapContent)) {
    violations.push({
...
function checkMilestoneStateCoherence(cwd) {
  ...
  if (activePhaseMatch && roadmapContent) {
    const activePhase = activePhaseMatch[1];
    const unpadded = String(parseInt(activePhase, 10));
    const phaseInRoadmap = new RegExp(
      `###\\s*Phase\\s+(?:${activePhase}|${unpadded})\\s*:`,
      'i'
    );
    if (!phaseInRoadmap.test(roadmapContent)) {
      violations.push({
