...
// ─── File Content Cache ───────────────────────────────────────────────────────
let _gatesRunCache = null;

function _gatesCachedRead(p) {
  if (!_gatesRunCache) return safeReadFile(p);
  if (!_gatesRunCache.has(p)) _gatesRunCache.set(p, safeReadFile(p));
  return _gatesRunCache.get(p);
}
...
function checkOrphanedPhases(cwd) {
  ...
  const roadmapContent = _gatesCachedRead(roadmapPath);
  ...
  try {
    ...
    if (err.code !== 'ENOENT') {
      process.stderr.write(`Warning: unexpected error reading phases directory: ${err.message}\n`);
    }
  }
...
function checkPhaseInRoadmap(cwd, phase) {
  ...
  const phaseRegex = new RegExp(`#{2,}\\s*Phase\\s+(?:${normalized}|${unpadded})\\s*:`, 'i');
...
function checkMilestoneStateCoherence(cwd) {
  ...
  if (activePhaseMatch && activeContent) {
    ...
    const phaseInRoadmap = new RegExp(`#{2,}\\s*Phase\\s+(?:${activePhase}|${unpadded})\\s*:`, 'i');
