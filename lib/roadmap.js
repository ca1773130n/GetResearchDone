...
function computeSchedule(cwd) {
  ...
  const roadmapContent = safeReadMarkdown(roadmapPath);
...
function cmdRoadmapGetPhase(cwd, phaseNum, raw) {
  ...
  try {
    const content = safeReadMarkdown(roadmapPath);
    if (!content) {
...
function analyzeRoadmap(cwd) {
  ...
  const content = safeReadMarkdown(roadmapPath);
  if (!content) {
    return { error: 'ROADMAP.md could not be read', milestones: [], phases: [], current_phase: null };
  }
  const activeContent = stripShippedSections(content);
  const phasesDir = getPhasesDirPath(cwd);
  ...
  while ((mMatchLocal = milestoneRegexLocal.exec(activeContent)) !== null) {
    ...
    const heading = mMatchLocal[1].trim();
    const afterHeading = activeContent.slice(mMatchLocal.index, mMatchLocal.index + 500);
    const startMatch = afterHeading.match(/\*\*Start:\*\*\s*(\d{4}-\d{2}-\d{2})/);
    milestonePositions.push({
      version,
      heading,
  ...
  let phasesDirNames = [];
  try {
    const phasesDirEntries = fs.readdirSync(phasesDir, { withFileTypes: true });
    phasesDirNames = phasesDirEntries.filter((e) => e.isDirectory()).map((e) => e.name);
  } catch (dirErr) {
    if (dirErr && dirErr.code && dirErr.code !== 'ENOENT') {
      process.stderr.write(
        `[roadmap] phases directory read error (${dirErr.code}): ${dirErr.message}\n`
      );
    }
  }
  ...
  while ((match = phasePattern.exec(activeContent)) !== null) {
    ...
    const lineNumber = activeContent.substring(0, match.index).split('\n').length;
    ...
    const durationMatch = section.match(/\*\*Duration:\*\*\s*(\d+)d/);
    const durationDays = durationMatch ? parseInt(durationMatch[1], 10) : defaultDuration;

    // Determine which milestone this phase belongs to.
    let phaseMilestone = milestonePositions.length > 0 ? milestonePositions[0].version : null;
    for (const ms of milestonePositions) {
    ...
    try {
      const dirMatch = phasesDirNames.find((d) => d.startsWith(normalized + '-') || d === normalized);
      ...
      if (scanErr && scanErr.code && scanErr.code !== 'ENOENT') {
        process.stderr.write(
          `[roadmap] phase directory scan error (${scanErr.code}): ${scanErr.message}\n`
        );
      }
    }
  ...
  const milestones = milestonePositions.map((ms) => ({ heading: ms.heading, version: ms.version }));
  ...
  for (const phase of phases) {
    if (phase._milestone !== currentMilestone) {
      currentMilestone = phase._milestone;
      const msStart = milestoneStartMap[currentMilestone];
      currentDate = msStart ? new Date(msStart + 'T00:00:00') : null;
    }
    phase.duration_days = phase._duration_days;
    if (currentDate) {
      phase.start_date = formatScheduleDate(currentDate);
      const endDate = addDays(currentDate, phase._duration_days - 1);
      phase.due_date = formatScheduleDate(endDate);
...
function cmdRoadmapAnalyze(cwd, raw) {
  ...
  output(result, raw, `${result.phase_count} phases, ${result.completed_phases} complete, ${result.progress_percent}% done`);
}
