...
function buildPlanPrompt(phaseNum) {
  return `Run /grd:plan-phase ${phaseNum}. Autonomous mode — make all decisions yourself, no questions. Complete all planning steps and write the PLAN.md files.`;
}
...
function buildExecutePrompt(phaseNum) {
  return `Run /grd:execute-phase ${phaseNum}. Autonomous mode — make all decisions yourself, no questions. After execution, merge locally. Do not push.`;
}
...
/**
 * Run the autopilot loop over a range of phases.
 * @param {string} cwd - Project working directory
 ...
 * @returns {{phases_attempted: number, phases_completed: number, stopped_at: string|null, results: Array}}
 */
function runAutopilot(cwd, options = {}) {
  const {
    from = null,
    to = null,
    resume = false,
  ...
  if (rangeError) {
    return { phases_attempted: 0, phases_completed: 0, stopped_at: rangeError, results: [] };
  }
  ...
  for (const phase of phases) {
    ...
    if (!skipPlan) {
      if (resume && isPhasePlanned(cwd, phaseNum)) {
        results.push({ phase: phaseNum, step: 'plan', status: 'skipped', reason: 'already planned' });
      } else if (dryRun) {
        results.push({ phase: phaseNum, step: 'plan', status: 'dry-run', prompt: buildPlanPrompt(phaseNum) });
      } else {
        writeStatusMarker(cwd, phaseNum, 'plan', 'started');
        updateStateProgress(cwd, phaseNum, 'planning');

        const planResult = spawnClaude(cwd, buildPlanPrompt(phaseNum), {
          timeout: timeoutMs,
          maxTurns,
          model,
        });
        ...
        if (planResult.exitCode !== 0) {
          ...
          writeStatusMarker(cwd, phaseNum, 'plan', 'failed');
          results.push({ phase: phaseNum, step: 'plan', status: 'failed', reason });
          stoppedAt = `Phase ${phaseNum} plan failed: ${reason}`;
          break;
        }

        // Verify plan files exist on disk
        if (!isPhasePlanned(cwd, phaseNum)) {
          writeStatusMarker(cwd, phaseNum, 'plan', 'failed');
          results.push({ phase: phaseNum, step: 'plan', status: 'failed', reason: 'no PLAN.md files found after planning' });
          stoppedAt = `Phase ${phaseNum} plan verification failed`;
          break;
        }

        writeStatusMarker(cwd, phaseNum, 'plan', 'completed');
        results.push({ phase: phaseNum, step: 'plan', status: 'completed' });
      }
    }
    ...
    if (!skipExecute) {
      if (resume && isPhaseExecuted(cwd, phaseNum)) {
        results.push({ phase: phaseNum, step: 'execute', status: 'skipped', reason: 'already executed' });
      } else if (dryRun) {
        results.push({ phase: phaseNum, step: 'execute', status: 'dry-run', prompt: buildExecutePrompt(phaseNum) });
      } else {
        ...
        // Verify execution produced summaries
        if (!isPhaseExecuted(cwd, phaseNum)) {
          writeStatusMarker(cwd, phaseNum, 'execute', 'failed');
          results.push({ phase: phaseNum, step: 'execute', status: 'failed', reason: 'no SUMMARY.md files found after execution' });
          stoppedAt = `Phase ${phaseNum} execute verification failed`;
          break;
        }
 ...
 */
function cmdAutopilot(cwd, args, raw) {
  ...
  const result = runAutopilot(cwd, options);
...
function cmdInitAutopilot(cwd, raw) {
  ...
  const incomplete = phases.filter(
    (p) => p.disk_status !== 'complete' && !p.roadmap_complete
  );
