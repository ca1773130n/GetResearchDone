 ...
 * @param {string|null} input - User input to match
 * @param {string[]} commands - List of valid command names
 * @returns {string|null} The closest command name, or null if too different
 */
...
function loadConfig(cwd) {
  ...
  try {
    ...
    if (parsed.model_profile !== undefined && !validProfiles.includes(parsed.model_profile)) {
      process.stderr.write(
    ...
    return {
      ...
      // Timeouts config
      timeouts: (() => {
        const t = parsed.timeouts && typeof parsed.timeouts === 'object' ? parsed.timeouts : {};
        const d = defaults.timeouts;
        return {
 ...
 *
 * @returns {{ init: function, reset: function, get: function(string, function): * }}
 */
function createRunCache() {
  ...
  return {
     ...
     * @param {string} key - Cache key (typically a file path)
     * @param {function(string): *} reader - Function to produce the value if not cached
 ...
 * @param {string} phaseArg - Phase identifier (e.g., '1', '01', '1.1')
 ...
 * @param {string} str - Directory name or phase string to parse
 * @returns {string|null} The numeric phase number as a string (e.g. '1', '1.2'), or null
 */
 ...
 * @param {string} rootDir - Root directory to walk (returned paths are relative to this)
 * @param {string[]} [excludePatterns=[]] - Substrings; any relative path containing one is skipped
 * @returns {string[]} Relative paths of all .js files found
 */
