...
const { safeReadFile, output, error, loadConfig, execGit, resolveModelForAgent, getMilestoneInfo } =
  require('./utils');
...
function discoverNewFeatureItems(cwd) {
  ...
  try {
    ...
    try {
      const integrationFiles = fs.readdirSync(integrationDir, { withFileTypes: true })
        .filter((e) => e.isFile() && e.name.endsWith('.js'))
        .map((e) => e.name);
  ...
  try {
    ...
    for (const file of libFiles) {
      ...
      while ((lMatch = loopPattern.exec(content)) !== null) {
        ...
        const isHeavy = /(?:spawnSync|execSync|spawnClaude|writeFileSync|appendFileSync)/.test(bodySlice);
...
async function runEvolve(cwd, options = {}) {
  ...
  while (iterCount < maxIter) {
    ...
    const discovery = runGroupDiscovery(discoveryCwd, state, effectivePickPct);
    ...
    for (const group of discovery.selected_groups) {
      ...
      for (const [step, promptFn] of [
        ...
        const result = spawnClaude(executionCwd, promptFn(group), {
          model: SONNET_MODEL,
          timeout: timeoutMs,
          maxTurns,
        });
  ...
  if (worktreeInfo) {
    ...
    const logResult = execGit(worktreeInfo.path, [
      'log', `${worktreeInfo.baseBranch}..HEAD`, '--oneline',
    ]);
...
async function cmdEvolve(cwd, args, raw) {
  ...
  const options = {
    ...
    pickPct: hasFlag('--pick-pct') ? parseInt(flag('--pick-pct', String(DEFAULT_PICK_PCT)), 10) : undefined,
...
function cmdEvolveDiscover(cwd, args, raw) {
  ...
  const discovery = runGroupDiscovery(cwd, previousState, pickPct);
  ...
  output(out, raw, raw ? `${discovery.groups_count} groups (${discovery.all_items_count} items)` : undefined);
}
