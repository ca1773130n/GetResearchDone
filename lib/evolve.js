 ...
 * Unexpected errors (not ENOENT) are logged to stderr.
 *
 * @param {string} name - Dimension name (used in error messages)
 * @param {Function} finder - Discoverer function: (cwd) => WorkItem[]
 * @param {string} cwd - Project working directory
 * @returns {WorkItem[]} Discovered items for this dimension (empty on error)
 */
function _discoverDimension(name, finder, cwd) {
  try {
    return finder(cwd);
  } catch (err) {
    ...
    if (err && err.code && err.code !== 'ENOENT') {
      process.stderr.write(
        `[evolve] discoverer error (${name}, ${err.code}): ${err.message}\n`
      );
    }
    return [];
  }
 ...
 *
 * Each dimension is discovered independently via _discoverDimension. A failure
 * in one dimension does not affect others (defensive try/catch per dimension).
 *
 * @param {string} cwd - Project working directory
 ...
 *
 * @param {Object} iterCtx - Iteration context
 * @param {string} iterCtx.discoveryCwd - CWD for discovery (original project root)
 * @param {string} iterCtx.executionCwd - CWD for execution (may be a worktree path)
 * @param {Object|null} iterCtx.state - Current evolve state
 * @param {boolean} iterCtx.useWorktree - Whether to use worktree isolation
 * @param {Object|null} iterCtx.worktreeInfo - Existing worktree info, or null
 * @param {number} iterCtx.effectivePickPct - Pick percentage for group selection
 * @param {boolean} iterCtx.dryRun - If true, discover but don't execute
 * @param {number|undefined} iterCtx.timeoutMs - Subprocess timeout in ms
 * @param {number|undefined} iterCtx.maxTurns - Max turns per subprocess
 * @param {string} iterCtx.cwd - Original project root (for worktree creation)
 * @param {Function} iterCtx.log - Log function
 * @returns {Promise<{discovery: Object, outcomes: Array, worktreeInfo: Object|null, executionCwd: string, useWorktree: boolean, isDryRun: boolean}>}
 */
...
/**
 * Process the result of a single iteration step: write evolution notes,
 * compute completed/failed groups, build state update, and build result record.
 *
 * @param {Object} stepResult - Return value of _runIterationStep
 * @param {Object} stepResult.discovery - Discovery result from runGroupDiscovery
 * @param {Array} stepResult.outcomes - Outcome objects per group
 * @param {Object|null} prevState - The state before this iteration (may be null)
 * @param {number} iterNum - Current iteration number
 * @param {number} effectivePickPct - Effective pick percentage
 * @param {string} cwd - Project root (evolution notes are always written here)
 * @returns {{ newState: Object, iterResult: Object }}
 */
function _handleIterationResult(stepResult, prevState, iterNum, effectivePickPct, cwd) {
  ...
  const newState = {
    ...
    history: prevState
      ? [...(prevState.history || []), historyEntry]
      : [historyEntry],
  };
