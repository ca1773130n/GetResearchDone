...
function discoverImproveFeatureItems(cwd) {
  ...
  try {
    ...
    for (const file of libFiles) {
      ...
      while ((oMatch = outputPattern.exec(content)) !== null) {
        // Check if it's output(data, raw) with no third arg (missing raw text format)
  ...
  try {
    ...
    for (const file of agentFiles) {
      ...
      // Check if agent has allowed_tools defined
      if (!frontmatter.includes('allowed_tools') && !frontmatter.includes('disallowed_tools')) {
        items.push(
          createWorkItem(
            'improve-features',
...
function discoverNewFeatureItems(cwd) {
  ...
  try {
    const commandsContent = safeReadFile(path.join(libDir, 'commands.js'));
    if (commandsContent) {
  ...
  try {
    ...
    for (const file of libFiles) {
      if (file === 'evolve.js') continue; // Skip self
      const content = safeReadFile(path.join(libDir, file));
      if (!content) continue;

      // Find hardcoded timeout values (> 1000ms)
  ...
  try {
    ...
    for (const file of libFiles) {
      ...
      // Find for-loops that process arrays and call async/heavy operations
...
/**
 * Discovery prompt sent to Claude subprocess.
 * Instructs Claude to analyze the codebase and output structured JSON work items.
 */
const DISCOVERY_PROMPT = `Analyze this codebase and find improvement opportunities. Output ONLY a JSON array.

Dimensions (pick the best fit for each item):
- productivity: Long functions, complex code, refactoring opportunities
- quality: Missing tests, TODO/FIXME markers, low coverage
- usability: Missing documentation, unclear APIs, poor error messages
- consistency: Inconsistent patterns, naming conventions, style violations
- stability: Swallowed errors, empty catch blocks, missing error handling
- improve-features: Features that could be better (UX, output, validation)
- new-features: Missing capabilities that would add value
...
For each item output:
{
  ...
  "title": "<short title>",
  "description": "<what to fix and why, include file path and line number>",
  "effort": "small|medium|large"
}

Rules:
- Scan ALL source files in the project, not just specific directories
- Be specific: include file paths and line numbers
- Focus on actionable improvements, not style nitpicks
- Output 20-50 items maximum
- Output ONLY the JSON array, no other text`;

/**
 * Discover improvement opportunities by running Claude as a subprocess.
 *
 * Falls back to the hardcoded `analyzeCodebaseForItems` if the Claude subprocess
 * fails (timeout, parse error, Claude not available, etc.).
 *
 * @param {string} cwd - Project working directory
 * @returns {Promise<WorkItem[]>} Discovered work items
 */
async function discoverWithClaude(cwd) {
  try {
    const model = resolveModelForAgent('grd-surveyor');
    const result = await spawnClaudeAsync(cwd, DISCOVERY_PROMPT, {
      captureOutput: true,
      model,
      maxTurns: 3,
      timeout: 120_000,
...
async function runEvolve(cwd, options = {}) {
  ...
  while (iterCount < maxIter) {
    ...
    log(
      `Discovered ${discovery.all_items_count} items in ${discovery.groups_count} groups, selected ${discovery.selected_groups.length} groups`
    );
    ...
    for (const group of discovery.selected_groups) {
      ...
      for (const [step, promptFn] of [
        ...
        const result = await spawnClaudeAsync(executionCwd, promptFn(group), {
          model: SONNET_MODEL,
          timeout: timeoutMs,
          maxTurns,
        });
