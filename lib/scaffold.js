...
const { planningDir: getPlanningDir, phasesDir: getPhasesDirPath, researchDir: getResearchDirPath } = require('./paths');
...
function cmdTemplateFill(cwd, templateType, options, raw) {
  if (!templateType) {
    error('template type required: summary, plan, or verification. Usage: scaffold <summary|plan|verification> --phase <N> --plan <M>. Specify a template type, e.g.: scaffold summary --phase 2 --plan 1');
  }
  if (!options.phase) {
...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    ...
    case 'baseline': {
      ...
      output({ created: true, path: path.relative(cwd, path.join(getPlanningDir(cwd), 'BASELINE.md')) }, raw, baselinePath);
      return;
    }
    default:
      error(
