...
function cmdTemplateSelect(cwd, planPath, raw) {
  ...
  try {
    ...
    const errorType = e && e.code === 'ENOENT' ? 'file_not_found' : 'read_error';
    output(
      { template: 'templates/summary-standard.md', type: 'standard', error: e.message, error_type: errorType },
      raw,
...
function cmdTemplateFill(cwd, templateType, options, raw) {
  if (!templateType) {
    error('template type required: summary, plan, or verification. Usage: scaffold <summary|plan|verification> --phase <N> --plan <M>');
  }
  if (!options.phase) {
  ...
  switch (templateType) {
    ...
    default:
      error(`Unknown template type: ${templateType}. Available: summary, plan, verification. Usage: scaffold <summary|plan|verification> --phase <N> --plan <M>`);
      return;
  }
...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    ...
    case 'phase-dir': {
      if (!phase || !name) {
        error('phase and name required for phase-dir scaffold. Usage: scaffold phase-dir --phase <N> --name <description>');
      }
    ...
    default:
      error(
        `Unknown scaffold type: ${type}. Available: context, uat, verification, phase-dir, research-dir, eval, baseline. Usage: scaffold <type> --phase <N>`
      );
  }
