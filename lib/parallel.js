 ...
 * @param {boolean} [options.nativeWorktreeAvailable] - When true, skip worktree_path pre-computation and set native_isolation flag per phase (defaults to false for backward compatibility)
 * @returns {Object} Structured context with mode, phases, status_tracker, backend info
 */
function buildParallelContext(cwd, phaseNumbers, options) {
  ...
  for (const phaseNum of phaseNumbers) {
    ...
    const wt_path = nativeWorktreeAvailable
      ? null
...
function cmdInitExecuteParallel(cwd, phaseNumbers, includes, raw) {
  ...
  output(context, raw, `Backend: ${context.backend}, ${context.phase_count} phases, mode: ${context.mode}`);
}
 ...
 *
 * @param {number} current - Current count (numerator)
 * @param {number} total - Total count (denominator)
 ...
 *
 * @param {string} phaseNum - Phase number (e.g. '3')
 * @param {number} current - Current plan number
 * @param {number} total - Total plans in phase
 * @param {string} [status] - Optional status label
 * @returns {void}
 */
function streamPhaseProgress(phaseNum, current, total, status) {
  const padded = String(phaseNum).padStart(2, '0');
  const bar = formatProgressBar(current, total);
  const statusPart = status ? ` [${status}]` : '';
  const line = `Phase ${padded}: ${current} of ${total} ${bar}${statusPart}\n`;
 ...
 * @param {boolean} raw - When false, writes to stderr; when true, outputs JSON
 * @returns {void}
 */
function cmdParallelProgress(args, raw) {
  // Parse args
  let phase = null;
  let plan = null;
  let totalPlans = null;
  ...
  const bar = formatProgressBar(plan, totalPlans);

  if (raw) {
    output({ phase, plan, total_plans: totalPlans, bar, status }, raw);
  } else {
    streamPhaseProgress(phase, plan, totalPlans, status);
  }
}
