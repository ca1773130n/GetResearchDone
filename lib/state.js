...
function cmdStateGet(cwd, section, raw) {
  ...
  try {
    const content = readStateFile(statePath);
 ...
 * @param {boolean} [opts.audit] - If true, appends audit entries to STATE.md
 * @returns {void} Outputs result to stdout and exits
 */
function cmdStatePatch(cwd, patches, raw, opts) {
  const options = opts || {};
  const statePath = path.join(cwd, '.planning', 'STATE.md');
  try {
    let content = readStateFile(statePath);
    ...
    for (const [field, value] of Object.entries(patches)) {
      ...
      if (pattern.test(content)) {
        ...
        if (normalizedField !== field) {
          const normalizedEscaped = normalizedField.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    ...
    if (options.audit && results.updated.length > 0) {
      ...
      const changedFields = results.updated.join(', ');
...
function cmdStateUpdate(cwd, field, value) {
  if (!field || value === undefined) {
    error('field and value required for state update. Usage: state patch --<field> <value>. Provide a field and value, e.g.: state patch --Status "in_progress"');
  }
 ...
 * @param {boolean} raw - Output raw text instead of JSON
 * @param {Object} [opts={}] - Options
 * @param {string} [opts.since] - ISO timestamp; if provided, returns diff since that snapshot
 * @returns {void} Outputs structured state snapshot to stdout and exits
 */
function cmdStateSnapshot(cwd, raw, opts) {
  const options = opts || {};
  const statePath = path.join(cwd, '.planning', 'STATE.md');
  ...
  let currentPhase = extractField('Active phase') || extractField('Current Phase');
  ...
  if (currentPhase) {
    const activePhaseMatch = currentPhase.match(/Phase\s+(\d+)\s+of\s+(\d+)\s*(?:\(([^)]+)\))?/i);
    if (activePhaseMatch) {
  ...
  const currentPlan = extractField('Current plan') || extractField('Current Plan');
  ...
  if (options.since) {
    const baselinePath = path.join(snapshotsDir, `${options.since}.json`);
    let baseline;
    try {
      baseline = JSON.parse(fs.readFileSync(baselinePath, 'utf-8'));
    } catch {
      output({ error: `No snapshot found for timestamp: ${options.since}`, full_snapshot: result }, raw);
      return;
    }

    // Compute changed scalar fields
    const changed_fields = {};
    for (const key of ['current_phase', 'status']) {
      const cur = result[key];
      const base = baseline[key];
      if (cur !== base) {
        changed_fields[key] = { from: base, to: cur };
      }
    }

    // Compute new decisions (in current but not in baseline)
    const baseDecisionSet = new Set(
      (baseline.decisions || []).map((d) => `${d.phase}|${d.summary}`)
    );
    const new_decisions = (result.decisions || []).filter(
      (d) => !baseDecisionSet.has(`${d.phase}|${d.summary}`)
    );

    // Compute new/resolved blockers
    const baseBlockerSet = new Set(baseline.blockers || []);
    const curBlockerSet = new Set(result.blockers || []);
    const new_blockers = (result.blockers || []).filter((b) => !baseBlockerSet.has(b));
    const resolved_blockers = (baseline.blockers || []).filter((b) => !curBlockerSet.has(b));
  ...
  output(result, raw, `Phase ${result.current_phase || 'none'} (${result.status || 'unknown'}) - ${result.progress_percent}%`);
}
