...
function formatNormalMilestoneList(milestones) {
  ...
  return milestones
    .map((m) => (m.note ? `${m.version} (${m.note})` : m.version))
    .join(', ');
}
  ...
  if (!parsed.milestones || parsed.milestones.length === 0) {
    ...
    for (const ms of parsed.milestones) {
      ...
      if (!VALID_STATUSES.includes(ms.status)) {
        errors.push(`${ms.id} has invalid status: ${ms.status}. Valid: ${VALID_STATUSES.join(', ')}`);
      }
    ...
    if (activeCount > 1) {
      warnings.push(`Multiple active LT milestones (${activeCount}). Consider having only one active.`);
    }
...
function generateLongTermRoadmap(milestones, projectName) {
  ...
  lines.push('|------|--------|---------|');
  lines.push(
    `| ${today} | Initial roadmap | Created ${milestones.length} LT milestones |`
  );
  lines.push('');
