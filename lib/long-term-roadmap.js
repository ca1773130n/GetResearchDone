...
function replaceSubsection(sectionText, headingPattern, newContent) {
  ...
  return sectionText.slice(0, headingEnd) + '\n' + newContent + '\n' + afterHeading.slice(endOffset);
}
...
function promoteMilestone(content, milestoneVersion) {
  const tier = getMilestoneTier(content, milestoneVersion);
  if (!tier) return { error: 'Milestone ' + milestoneVersion + ' not found' };
  if (tier === 'now') return { error: 'Milestone ' + milestoneVersion + ' is already in Now tier and cannot be promoted further' };
  ...
  if (tier === 'later') {
    ...
    const nextH = afterHeading.match(/^#{2,3}\s/m);
    const removeEnd = nextH
      ? removeStart + headingMatch[0].length + nextH.index
      : content.length;
  ...
  if (tier === 'next') {
    ...
    const nextH2 = afterNow.match(/^##\s/m);
    const nowEnd = nextH2
      ? nowStart + nowMatch[0].length + nextH2.index
      : result.length;
