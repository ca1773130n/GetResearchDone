...
function parseTierMilestones(sectionText, tier) {
  ...
  for (let i = 0; i < milestoneMatches.length; i++) {
    ...
    if (tier === 'next') {
      ...
      const open_research_questions = extractBulletList(milestoneText, 'Open Research Questions');
...
function generateLongTermRoadmap(milestones, projectName, planningHorizon) {
  ...
  const laterMs = milestones.filter((m, i) => i > 0 && !nextMs.includes(m));
...
function formatLongTermRoadmap(parsed) {
  ...
  if (parsed.now) {
    const n = parsed.now;
    lines.push(
      `[Now]  ${n.version || '?'} - ${n.milestone?.replace(n.version, '').replace(/^\s*-\s*/, '') || 'Current'}  Status: ${n.status || 'Unknown'}`
    );
  ...
  if (parsed.next && parsed.next.length > 0) {
    for (const ms of parsed.next) {
      lines.push(
        `[Next]  ${ms.version || '?'} - ${ms.milestone?.replace(ms.version, '').replace(/^\s*-\s*/, '') || 'Upcoming'}  Status: ${ms.status || 'Next'}`
      );
  ...
  if (parsed.later && parsed.later.length > 0) {
    for (const ms of parsed.later) {
      lines.push(
        `[Later] ${ms.version || '?'} - ${ms.milestone?.replace(ms.version, '').replace(/^\s*-\s*/, '') || 'Future'}  Timeline: ${ms.estimated_timeline || 'TBD'}`
      );
      if (ms.goal) lines.push(`        ${ms.goal}`);
      if (ms.dependencies) lines.push(`        Depends on: ${ms.dependencies}`);
      lines.push('');
 ...
 * @param {string} milestoneVersion - Version string to look up (e.g., 'v0.2.0')
 * @returns {'now'|'next'|'later'|null} Tier name, or null if not found
 */
function getMilestoneTier(content, milestoneVersion) {
  const parsed = parseLongTermRoadmap(content);
  if (!parsed) return null;

  if (parsed.now && parsed.now.version === milestoneVersion) return 'now';
  if (parsed.next && parsed.next.some((m) => m.version === milestoneVersion)) return 'next';
  if (parsed.later && parsed.later.some((m) => m.version === milestoneVersion)) return 'later';
...
/**
 * Replace content within a subsection (heading to next heading or section end).
 * @param {string} sectionText - Section markdown text
 * @param {RegExp} headingPattern - Pattern to match the subsection heading
 * @param {string} newContent - Replacement content (lines after heading)
 * @returns {string} Updated section text
 */
function replaceSubsection(sectionText, headingPattern, newContent) {
  const match = sectionText.match(headingPattern);
  if (!match) return sectionText;

  const headingEnd = match.index + match[0].length;
  const afterHeading = sectionText.slice(headingEnd);

  // Find the next heading (any level) or bold field or --- separator
  const nextBoundary = afterHeading.match(/^(#{2,4}\s|---\s*$|\*\*[A-Z])/m);
  const endOffset = nextBoundary ? nextBoundary.index : afterHeading.length;

  return (
    sectionText.slice(0, headingEnd) + '\n' + newContent + '\n' + afterHeading.slice(endOffset)
  );
}
 ...
 * @param {string} content - Raw LONG-TERM-ROADMAP.md content
 * @param {string} milestoneVersion - Version string to refine
 * @param {Object} updates - Partial updates: { goal?, success_criteria?, rough_phase_sketch?, open_questions?, open_research_questions? }
 * @returns {string|Object} Updated content string, or { error } if milestone not found
 */
function refineMilestone(content, milestoneVersion, updates) {
  const tier = getMilestoneTier(content, milestoneVersion);
  if (!tier) return { error: 'Milestone ' + milestoneVersion + ' not found' };

  // Find the milestone section boundaries in raw content
  let sectionStart, sectionEnd;

  if (tier === 'now') {
    // Now section: ## Current Milestone (Now) to next ## heading
    const nowMatch = content.match(/^##\s+Current Milestone\s*\(Now\)/m);
    if (!nowMatch) return { error: 'Now section not found' };
    sectionStart = nowMatch.index;
    const afterNow = content.slice(sectionStart + nowMatch[0].length);
    const nextH2 = afterNow.match(/^##\s/m);
    sectionEnd = nextH2 ? sectionStart + nowMatch[0].length + nextH2.index : content.length;
  } else {
    // Next or Later: ### {version} - {name} subsection
    const parsed = parseLongTermRoadmap(content);
    const milestone =
      tier === 'next'
        ? parsed.next.find((m) => m.version === milestoneVersion)
        : parsed.later.find((m) => m.version === milestoneVersion);

    if (!milestone) return { error: 'Milestone ' + milestoneVersion + ' not found in parsed data' };

    // Find the ### heading for this milestone
    const escapedVersion = milestoneVersion.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const headingPattern = new RegExp(`^###\\s+${escapedVersion}\\s*-\\s*`, 'm');
    const headingMatch = content.match(headingPattern);
    if (!headingMatch) return { error: 'Milestone heading not found for ' + milestoneVersion };
    ...
    const nextH = afterHeading.match(/^#{2,3}\s/m);
    sectionEnd = nextH ? sectionStart + headingMatch[0].length + nextH.index : content.length;
  }

  let section = content.slice(sectionStart, sectionEnd);

  // The heading level for subsections depends on tier
  const subLevel = tier === 'now' ? '###' : '####';

  if (updates.goal !== undefined) {
    const goalPattern = new RegExp(`^${subLevel}\\s+Goal`, 'm');
    section = replaceSubsection(section, goalPattern, updates.goal + '\n');
  }

  if (updates.success_criteria !== undefined) {
    const scPattern = new RegExp(`^${subLevel}\\s+Success Criteria`, 'm');
    const newList = updates.success_criteria.map((c) => `- ${c}`).join('\n') + '\n';
    section = replaceSubsection(section, scPattern, newList);
  }

  if (updates.rough_phase_sketch !== undefined) {
    const rpsPattern = new RegExp(`^${subLevel}\\s+Rough Phase Sketch`, 'm');
    const newList = updates.rough_phase_sketch.map((p, i) => `${i + 1}. ${p}`).join('\n') + '\n';
    section = replaceSubsection(section, rpsPattern, newList);
  }

  if (updates.open_questions !== undefined) {
    const oqPattern = new RegExp(`^${subLevel}\\s+Open Questions`, 'm');
    const newList = updates.open_questions.map((q) => `- ${q}`).join('\n') + '\n';
    section = replaceSubsection(section, oqPattern, newList);
  }

  if (updates.open_research_questions !== undefined) {
    const orqPattern = new RegExp(`^${subLevel}\\s+Open Research Questions`, 'm');
    const newList = updates.open_research_questions.map((q) => `- ${q}`).join('\n') + '\n';
    section = replaceSubsection(section, orqPattern, newList);
  }

  return content.slice(0, sectionStart) + section + content.slice(sectionEnd);
}
 ...
 * @param {string} content - Raw LONG-TERM-ROADMAP.md content
 * @param {string} milestoneVersion - Version string to promote
 * @returns {string|Object} Updated content string, or { error } if promotion not possible
 */
function promoteMilestone(content, milestoneVersion) {
  const tier = getMilestoneTier(content, milestoneVersion);
  if (!tier) return { error: 'Milestone ' + milestoneVersion + ' not found' };
  if (tier === 'now')
    return {
      error:
        'Milestone ' + milestoneVersion + ' is already in Now tier and cannot be promoted further',
    };
  ...
  if (tier === 'later') {
    ...
    const milestone = parsed.later.find((m) => m.version === milestoneVersion);
    if (!milestone) return { error: 'Milestone not found in Later tier' };

    // Remove the milestone's subsection from Later section
    const escapedVersion = milestoneVersion.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const headingPattern = new RegExp(`^###\\s+${escapedVersion}\\s*-\\s*[^\\n]+`, 'm');
    const headingMatch = content.match(headingPattern);
    if (!headingMatch) return { error: 'Later milestone heading not found' };

    const removeStart = headingMatch.index;
    const afterHeading = content.slice(removeStart + headingMatch[0].length);
    const nextH = afterHeading.match(/^#{2,3}\s/m);
    const removeEnd = nextH ? removeStart + headingMatch[0].length + nextH.index : content.length;

    let result = content.slice(0, removeStart) + content.slice(removeEnd);

    // Build new Next-tier subsection
    const nameMatch = milestone.milestone.match(/v[\d.]+\s*-\s*(.+)/);
    const name = nameMatch ? nameMatch[1].trim() : milestone.milestone;
    const lines = [];
    lines.push(`### ${milestoneVersion} - ${name}`);
    lines.push('');
    lines.push(`**Status:** Next`);
    lines.push(`**Estimated Start:** TBD`);
    lines.push(`**Estimated Duration:** TBD`);
    if (milestone.dependencies) lines.push(`**Dependencies:** ${milestone.dependencies}`);
    lines.push('');
    lines.push('#### Goal');
    lines.push(milestone.goal || 'TBD');
    lines.push('');
    if (milestone.success_criteria && milestone.success_criteria.length > 0) {
      lines.push('#### Success Criteria');
      for (const c of milestone.success_criteria) {
        lines.push(`- ${c}`);
      }
      lines.push('');
    }
    ...
    lines.push('');
    lines.push('#### Open Questions');
    if (milestone.open_research_questions && milestone.open_research_questions.length > 0) {
      for (const q of milestone.open_research_questions) {
        lines.push(`- ${q}`);
      }
    } else {
      lines.push('- TBD');
    ...
    const newSubsection = lines.join('\n');

    // Find the end of the Next Milestones section to append
    const nextSectionMatch = result.match(/^##\s+Next Milestones/m);
    if (!nextSectionMatch) return { error: 'Next Milestones section not found' };

    const afterNextHeader = result.slice(nextSectionMatch.index + nextSectionMatch[0].length);
    const nextH2 = afterNextHeader.match(/^##\s/m);
    const insertPoint = nextH2
      ? nextSectionMatch.index + nextSectionMatch[0].length + nextH2.index
      : result.length;

    result = result.slice(0, insertPoint) + newSubsection + result.slice(insertPoint);
  ...
  if (tier === 'next') {
    // Next -> Now promotion
    const milestone = parsed.next.find((m) => m.version === milestoneVersion);
    if (!milestone) return { error: 'Milestone not found in Next tier' };

    // Remove the milestone's subsection from Next section
    const escapedVersion = milestoneVersion.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const headingPattern = new RegExp(`^###\\s+${escapedVersion}\\s*-\\s*[^\\n]+`, 'm');
    const headingMatch = content.match(headingPattern);
    if (!headingMatch) return { error: 'Next milestone heading not found' };
    ...
    // Build new Now section
    const nameMatch = milestone.milestone.match(/v[\d.]+\s*-\s*(.+)/);
    const name = nameMatch ? nameMatch[1].trim() : milestone.milestone;
    const today = new Date().toISOString().split('T')[0];
    const startDate = milestone.estimated_start || today;

    const lines = [];
    lines.push('## Current Milestone (Now)');
    lines.push('');
    lines.push(`**Milestone:** ${milestoneVersion} - ${name}`);
    ...
    const newNowSection = lines.join('\n');

    // Replace the existing Now section
    const nowMatch = result.match(/^##\s+Current Milestone\s*\(Now\)/m);
    if (!nowMatch) return { error: 'Current Milestone (Now) section not found' };

    const nowStart = nowMatch.index;
    const afterNow = result.slice(nowStart + nowMatch[0].length);
    const nextH2 = afterNow.match(/^##\s/m);
    const nowEnd = nextH2 ? nowStart + nowMatch[0].length + nextH2.index : result.length;

    result = result.slice(0, nowStart) + newNowSection + result.slice(nowEnd);

    return result;
  }

  return { error: 'Unexpected tier: ' + tier };
}

// ─── Refinement History Update ───────────────────────────────────────────────

/**
 * Append a row to the Refinement History markdown table.
 * @param {string} content - Raw LONG-TERM-ROADMAP.md content
 * @param {string} action - Action description (e.g., 'Refined', 'Promoted', 'Added')
...
function updateRefinementHistory(content, action, details) {
  ...
  // Find the last table row in the Refinement History section
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (i > 0 && trimmed.startsWith('#')) break; // Next section
    if (trimmed.startsWith('|')) {
      inTable = true;
      lastTableLineIdx = i;
