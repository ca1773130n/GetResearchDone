...
const { execGit, output, error, getMilestoneInfo, loadConfig, findPhaseInternal } = require('./utils');
...
function cmdWorktreePushAndPR(cwd, options, raw) {
  ...
  try {
    ...
    const stderr = ghErr.stderr ? ghErr.stderr.toString().trim() : ghErr.message || 'Unknown gh error';
    output({
