...
function cmdWorktreeCreate(cwd, options, raw) {
  ...
  if (!options.force) {
    const branchCheck = execGit(cwd, ['rev-parse', '--verify', 'refs/heads/' + branch]);
...
function cmdWorktreeRemove(cwd, options, raw) {
  ...
  if (options.path) {
    ...
    error('Either --phase or --path is required for worktree remove. Usage: worktree remove --phase <N> OR worktree remove --path <worktree-path>');
  }
  ...
  const removeResult = execGit(cwd, ['worktree', 'remove', wtPath, '--force'], { allowBlocked: true });
  if (removeResult.exitCode !== 0) {
    process.stderr.write('Warning: git worktree remove failed: ' + (removeResult.stderr || 'unknown error').trim() + '\n');
  }
 ...
 *
 * @param {string} cwd - Project working directory (must be a git repo)
 * @returns {{ path: string, branch: string, baseBranch: string, created: string } | { error: string }}
 */
 ...
 * @returns {{ removed: boolean, already_gone?: boolean } | { error: string }}
 */
 ...
 * @param {string} [options.body] - PR body markdown
 * @param {string} [options.base] - Base branch for PR target
 * @returns {{ pr_url: string, branch: string, base: string } | { error: string, push_succeeded?: boolean }}
 */
function pushAndCreatePR(cwd, wtPath, options = {}) {
  ...
  const pushResult = execGit(wtPath, ['push', '-u', 'origin', branch], { allowBlocked: true });
  if (pushResult.exitCode !== 0) {
    return { error: `Failed to push branch: ${pushResult.stderr}`, push_succeeded: false };
  }
...
function cmdWorktreeMerge(cwd, options, raw) {
  ...
  if (!phase) {
    error('phase is required for worktree merge. Usage: worktree merge --phase <N>');
  }
  ...
  if (phaseMatch) {
    ...
    try {
      ...
      if (grdBranch !== wtBranch) {
        ...
        process.stderr.write('Warning: branch rename failed: ' + (renameResult.stderr || 'git branch rename failed').trim() + '\n');
        output(
          {
            hooked: true,
            worktree_path: wtPath,
