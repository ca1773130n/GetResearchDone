 ...
 * @param {string} [options.branch] - Explicit branch name (overrides computed branch from template)
...
function cmdWorktreeHookCreate(cwd, wtPath, wtBranch, raw) {
  ...
  if (phaseMatch) {
    ...
    try {
      const milestone = getMilestoneInfo(cwd);
      const phaseInfo = findPhaseInternal(cwd, phase);
      const slug = phaseInfo ? generateSlugInternal(phaseInfo.phase_slug || phaseInfo.phase_name) : phase;
