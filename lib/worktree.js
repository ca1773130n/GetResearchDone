 ...
 * at {project}/.worktrees/{milestone}-{phase}.
 *
 ...
 * Uses the project-local worktree_dir from config (default: .worktrees/).
 * @param {string} cwd - Project working directory
 * @param {string} milestone - Milestone version (e.g., 'v0.2.0')
 * @param {string} phase - Phase number (e.g., '27')
 * @returns {string} Absolute path under project's worktree directory
 */
 ...
 * Expected format: {milestone}-{phase} (e.g., v0.2.0-27)
 *
 ...
 * Scans the project-local worktree directory (.worktrees/ by default).
 * @param {string} cwd - Project working directory
...
function getGrdWorktrees(cwd) {
  ...
  const config = loadConfig(cwd);
  const wtDir = path.resolve(cwd, config.worktree_dir || '.worktrees/');

  return all
    .filter((wt) => wt.path.startsWith(wtDir))
    .map((wt) => {
...
/**
 * Ensure a .gitignore entry exists in the project root.
 * Creates .gitignore if it does not exist. Appends entry if not already present.
 * @param {string} cwd - Project working directory
 * @param {string} entry - The gitignore entry to add (e.g., '.worktrees/')
 * @returns {boolean} True if entry was added, false if already present
 */
function ensureGitignoreEntry(cwd, entry) {
  ...
  // Check if entry already present (exact line match, ignoring trailing slash)
 ...
 *
 * Creates a worktree at {project}/.worktrees/{milestone}-{phase}
 * with a branch following the configured phase_branch_template pattern.
 * Auto-injects .worktrees/ into .gitignore on first creation.
 *
...
function cmdWorktreeCreate(cwd, options, raw) {
  ...
  // Auto-inject .worktrees/ into .gitignore
  const config = loadConfig(cwd);
  ensureGitignoreEntry(cwd, config.worktree_dir || '.worktrees/');
 ...
 * GRD-created worktrees (those in the project-local .worktrees/ directory).
...
function cmdWorktreePushAndPR(cwd, options, raw) {
  ...
  // Resolve base branch using strategy-aware targeting
  const milestoneInfo = getMilestoneInfo(cwd);
  const milestoneSlug = generateSlugInternal(milestoneInfo.name);
  const baseBranch = options.base || resolveTargetBranch(cwd, milestone, milestoneSlug);
...
/**
 * Create a milestone branch from base_branch after verifying HEAD is on base_branch.
 * Returns a result object (does not call output/error/process.exit) for testability.
 *
 * @param {string} cwd - Project working directory
 * @param {string} milestone - Milestone version (e.g., 'v0.2.3')
 * @param {string} slug - Milestone slug (e.g., 'improve-settings')
 * @returns {Object} Result with {branch, created, base} on success, or {error, ...} on failure
 */
...
/**
 * Resolve the PR target branch based on branching strategy.
 * - 'milestone': phase PRs target the milestone branch
 * - 'phase' or 'none' or fallback: PRs target base_branch directly
 *
 * Returns a result (does not call output/error/process.exit) for testability.
 *
 * @param {string} cwd - Project working directory
 * @param {string} milestone - Milestone version
 * @param {string} slug - Milestone slug
 * @returns {string} Target branch name for PRs
 */
function resolveTargetBranch(cwd, milestone, slug) {
  ...
  if (strategy === 'milestone') {
    ...
    const template = config.milestone_branch_template || 'grd/{milestone}-{slug}';
    return template.replace('{milestone}', milestone).replace('{slug}', slug || milestone);
  }
