...
function runTestGate(wtPath, testCmd) {
  ...
  // Use execSync with shell because test commands are user-specified shell
  // expressions (e.g. "npm test", "npx jest ...") from GRD plan config.
  // This is safe: testCmd comes from .planning/ config, not user input.
  try {
 ...
 * @returns {{ discarded: boolean, path: string, branch: string, branch_deleted: boolean, cleanup_error?: string }}
 */
 ...
 *
 * For merge/pr, cleanup runs in a finally block to ensure it happens even
 * if an error occurs mid-operation. For discard, cleanup is handled by
 * discardWorktree() internally.
 *
 * @param {string} cwd - Project working directory (main repo, NOT worktree)
 * @param {Object} options
...
function cmdWorktreeComplete(cwd, options, raw) {
  ...
  // Tests passed -- proceed with action
  if (action === 'merge') {
    ...
    const actionResult = mergeWorktree(cwd, { branch, baseBranch });
      ...
      execGit(cwd, ['branch', '-d', branch], { allowBlocked: true });
    ...
    if (pushResult.exitCode !== 0) {
      // Cleanup before exiting — output() calls process.exit() so finally blocks won't run
      cleanupWorktree(cwd, wtPath);
      output(
        {
          action: 'pr',
      ...
      try {
        ...
        const stderr = ghErr.stderr
          ? ghErr.stderr.toString().trim()
          : ghErr.message || 'Unknown gh error';
        // Cleanup before exiting — output() calls process.exit()
        cleanupWorktree(cwd, wtPath);
        output(
          {
            action: 'pr',
    ...
    // Cleanup before output — output() calls process.exit() so finally blocks won't run
    cleanupWorktree(cwd, wtPath);
    output(
      {
        action: 'pr',
